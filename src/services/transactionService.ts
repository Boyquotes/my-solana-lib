...
import { CyberGoldSDK }            from '../index';
...
export class TransactionService {
    ...
    public async mintFromSol(
        ...
        const allSigners = [...stakeSigs, ...mintSigs].filter((s: unknown): s is Keypair => s instanceof Keypair);
    ...
    public async redeem(
        ...
        const { tx: burnTx, signers: burnSigners } = await this.sdk.prepareTransaction(
            burnIxs,
            feePayer,
            burnSigs.filter((s: unknown): s is Keypair => s instanceof Keypair)
        );
    ...
    public async liquidate(
        ...
        const { tx: liquidateTx, signers: liquidateSigners } = await this.sdk.prepareTransaction(
            liquidateIxs,
            feePayer,
            liquidateSigs.filter((s: unknown): s is Keypair => s instanceof Keypair)
        );
